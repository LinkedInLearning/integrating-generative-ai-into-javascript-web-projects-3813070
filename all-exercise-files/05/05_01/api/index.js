...
const retrieveMessages = async (run, thread) => {
  if(run.status === "completed"){
    ...
    return {role: "assistant", content: messages.data[0].content[0].text.value} 
  }
};
...
const {assistant, thread } = main();

async function sendMessage(req, res) { 
  await addMessageToThread(thread, req.body.userMessage);
  ...
  // Step 6: Retrieve and display the Messages
  const message =  await retrieveMessages(run, thread);
